...
REMEMBER in regards to the wording:\newline%
	--- \xspace A variable is NOT a data member \newline%
	- \xspace\xspace\xspace\xspace\xspace In \tcode{struct A \{ int d; \};} \tcode{d} is not a variable,\newline%
	- \xspace\xspace\xspace\xspace\xspace and potentially evaluated expressions and odr-use\newline 
	- \xspace\xspace\xspace\xspace\xspace refer only to variables.\newline
%	
	--- \xspace Only if 'x' is an object do we require the lvalue-to-rvalue conversion on 'e' to avoid odr-use, but if 'x' is a reference that is initialized with a constant expression, then it will not be odr-used.\newline%
	--- \xspace 'ex' is like DeclRefExpr (clang) it is the id-expression within an 'e' that names 'x'\newline
	--- \xspace 'e' is not the full expression - but any contained expression 'e' within the full-expression that has 'ex' as its potential results - and lval-to-rval conversion is applied - the goal is to find such an 'e' that has such an 'ex' as its potential result%
}
...
void g() {
  ...
  {
    extern void f();            // internal linkage
    extern int i;               // \#3 external linkage, ill-formed \fvins{rationale: \fvcwg{426}}
  }
}
\end{codeblock}
...
\fvmargin{Objects are only created by definitions, new-expressions or as temporaries.  \tcode{malloc} does not create an object.  Also, definition of object was modified. See \fvcwg{1776} and papers \fvpaper{n3751}, \fvpaper{P0137}, \fvcwg{2182}, which makes vector::data unimplementable since while it uses an underlying char array it does not have an array object of T, so how can you do pointer arithmetic with it}
...
subobjects may have zero size. \fv{An object of trivially copyable or
...
\fvmargin{ ?? side effects are not the same as observable behavior.  They are used solely to specify the sequenced before relationship \fvcwg{n2239} \fvcwg{637}.}
...
\fvmargin{A modification order does not imply happens-before, though it must be consistent with happens-before.  That is the following program still has a data-race: \newline \newline%
	...
	\tcode{if (a.load(relaxed)==0) %
		\{\newline
		\mbox{ \xspace\xspace payload = 3.14;}\newline 
		\mbox{ \xspace\xspace ai.store(1, relaxed);}\newline%
		\}}%
	\newline\newline T2:\newline %
	\tcode{if (a.load(relaxed)==1) %
		\newline \mbox{\xspace\xspace payload += 1;}%
}} 
...
before \placeholder{A}.\begin{fvnote}{This does not mean that \placeholder{A} happens-before \placeholder{B}}\end{fvnote}
\begin{note}
