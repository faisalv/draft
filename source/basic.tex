...
\end{itemize}
\fvmargin{"The C++14 wording for odr-use works as follows: normally, any mention of a variable in a potentially-evaluated expression would be an odr-use and would require a definition. However, for a variable whose value can be read in a constant expression ("satisfies the requirements for appearing in a[n lvalue-to-rvalue conversion in a] constant expression"), and for which only the value (and not the address or storage) is needed, a definition (or, for a lambda, a capture) is not required. In practice, in such cases, a compiler would emit the value directly rather than emitting a reference to the object.	\newline\newline The rest of the relevant wording is explaining the circumstances under which an implementation is required to determine that only the value is needed. This is the case if we can:\newline1) start from either an lvalue-to-rvalue conversion or a discarded value expression,\newline2) walk over constructs which clearly don't introduce a use of the value's address or storage (those listed in 3.2/2), and\newline3) find the use of the variable."-rsmith\newline\newline%
REMEMBER in regards to the wording:\newline%
	--- \space A variable is NOT a data member \newline%
	- \space\space\space\space\space In \tcode{struct A \{ int d; \};} \tcode{d} is not a variable,\newline%
	- \space\space\space\space\space and potentially evaluated expressions and odr-use\newline%
	- \space\space\space\space\space refer only to variables.\newline%
%	
	--- \space Only if 'x' is an object do we require the lvalue-to-rvalue conversion on 'e' to avoid odr-use, but if 'x' is a reference that is initialized with a constant expression, then it will not be odr-used.\newline%
	--- \space 'ex' is like DeclRefExpr (clang) it is the id-expression within an 'e' that names 'x'\newline%
	--- \space 'e' is not the full expression - but any contained expression 'e' within the full-expression that has 'ex' as its potential results - and lval-to-rval conversion is applied - the goal is to find such an 'e' that has such an 'ex' as its potential result%
}
...
\fvmargin{While using the dot-operator does require a class to be complete, using the scope resolution operator does not! See \fvcwg{287} and John Spicer's comments: \textit{Using a class to form a qualified name does not require the class to be complete, it only requires that the named member already have been declared. ... A member's potential scope begins at its point of declaration. In other words, a class has three states: incomplete, being completed, and complete. The standard permits a qualified name to be used once a name has been declared. The quotation of the notes about the potential scope was intended to support that.}}
...
\fvmargin{\fvpaper{p1787}
	Davis's omnibus paper re-engineered name lookup wording by addressing the following general issues: \newline
	
	[basic.scope] defined the word “scope” in two different ways, and then implicitly and indirectly specified many aspects of name lookup with imperfect correspondence to the descriptions in [basic.lookup]. Lookup was often described for units of program text that might include multiple names and < tokens that must be disambiguated, such as a template-id ([temp.dep]/2), id-expression ([over.call.func]/2), or conversion-type-id ([class.qual]/1.2). Namespaces, and the global namespace in particular, were inconsistently described as being one or many declarative regions ([basic.scope.namespace]/1, /4).\newline\newline
	
	Modules significantly complicated the situation, principally by invalidating the tacit assumption that “every” lookup is restricted to preceding declarations in the same translation unit. There were already exceptions to that rule (e.g., complete class contexts and using-declaration conflicts), which could then easily be (mis)read as applying to all contents of all translation units. Many rules, like those for lookup through a using-directive, did not make any mention of location at all, and so became  ambiguous.\newline\newline
	
	The similar process of associating a declaration with a previous declaration of the same name was also poorly specified. In particular, there was no consistent rule as to whether multiple declarations of the same name introduced distinct entities (that might conflict) or one entity (and might conflict).\newline\newline
...
\fvmargin{Rationale: \fvcwg{39} \fvpaper{n1626} \fvpaper{n1543}}
...
\fvmargin{An extern function in an unnamed namespace has internal linkage.  \href{https://groups.google.com/a/isocpp.org/d/msg/std-discussion/oyLCkCUKpfU/vgw7DPMhFgAJ}{rsmith's example}}
\pnum
In addition,
a member function,
...
\pnum
\fvmargin{There is no semantic difference between a local function declaration declared with our without \tcode{extern} \href{http://lists.isocpp.org/core/2016/08/0826.php}{Hubert's reply}}
\begin{example}
\begin{codeblock}
static void f();
extern "C" void h();
...
\fvmargin{Use zero-sized bit fields, or nest a data member within a nested struct to prevent conflicting access}
\pnum
\begin{example}
A class declared as
\begin{codeblock}
...
\fvmargin{Objects are only created by definitions, new-expressions or as temporaries.  \tcode{malloc} does not create an object.  Also, definition of object was modified. See \fvcwg{1776} and papers \fvpaper{P0593} \fvpaper{n3751}, \fvpaper{P0137}, \fvcwg{2182}, which makes vector::data unimplementable since while it uses an underlying char array it does not have an array object of T, so how can you do pointer arithmetic with it}
...
\fv{An object of trivially copyable or
...
\begin{note}
\fv{This latitude is granted to allow objects of class type to be passed to or returned from functions in registers.}
\end{note}
\fvmargin{\href{http://lists.isocpp.org/core/2016/07/0772.php}{See discussion by Jeffrey and Richard reflector-cwg-772:}\newline%
	It's this copy that the ABI requires be performed today, and that the standard disallows if the type contains a volatile member. Example:\newline\newline%
\mbox{\space\space\tcode{struct X \{ X *volatile p = this; \};}}\newline%
\mbox{\space\space\tcode{void f(X x) \{ assert(x.p == \&x); \}}}\newline%
\mbox{\space\space\tcode{void g() \{ f(\{\}); \}}}\newline%
\newline\newline%
If X has a trivial copy or move constructor, a technical copy may be performed as part of the call to f(X), and the assert may fail. Otherwise, the assert must not fail, because the object constructed in g() is the same object as f's parameter.%
\newline \newline %
Current ABIs, such as the x86_64 ABI, pass X in registers.%
\fvcwg{496} \fvcwg{2094}
}
...
\pnum
\indextext{value computation|(}%
\fvmargin{side effects are not the same as observable behavior.  They are used solely to specify the sequenced before relationship \fvcwg{2239} \fvcwg{637} \href{https://stackoverflow.com/questions/13271469/how-are-side-effects-and-observable-behavior-related-in-c}{stack-overflow-answer}}%
\ignorespaces
...
\fvmargin{A modification order does not imply happens-before, though it must be consistent with happens-before.  That is the following program still has a data-race: \newline \newline%
		\tcode{atomic<int> a;\newline double payload;}%
		\newline \newline T1:\newline %
		\tcode{if (a.load(relaxed)==0) %
				\{\newline
				\mbox{ \space\space payload = 3.14;}\newline 
				\mbox{ \space\space ai.store(1, relaxed);}\newline%
				\}}%
		\newline\newline T2:\newline %
		\tcode{if (a.load(relaxed)==1) %
				\newline \mbox{\space\space payload += 1;}\newline%
		}} 
...
before $A$.
\fvmargin{This does not mean that \placeholder{A} happens-before \placeholder{B}}
\begin{note}
