...
and \fvins{assuming} the type of the identifier in the declaration
\tcode{T}
\tcode{D1}
...
\begin{fvnote}
  \textit{derived-declarator-type-list} is the assumed known variable that is used to recursively determine the description of a type through syntactic pattern-matching, decomposition and recomposition. In spite of the italics it is NOT a grammar term. It is used as follows: Say one wants to determine the full type of a variable declared with a complicated combination of declarator-operators (i.e. *, \&, (), []), then assuming that we know how the inner declarators have affected the type (since we start calculation using the inner most declarator-operators, i.e. those closest to the identifier) --- i.e. assume the resulting type using \tcode{'N'} inner declarators is {\color{addclr}{'\textit{derived-declarator-type-list} \tcode{T}'}}, then using the current declarator ('*') we can determine that the current type (i.e. then for \tcode{'N+1'} is {\color{addclr}'\textit{derived-declarator-type-list} {\color{orange} pointer to} \tcode{T}'}).  \term{derived-declarator-type-list} consists of some valid concatenated sequence of the following phrases [that ends with \tcode{T}]: \begin{enumerate}
  	...
  	\item \tcode{noexcept}\opt function of (\grammarterm{parameter-declaration-clause} )
  	\grammarterm{cv-qualifier-seq\opt} \grammarterm{ref-qualifier\opt} returning'
  \end{enumerate}
  	...
  	// \xspace\xspace\xspace   --- \tcode{T = 'const int'} and \tcode{D = '*const *volatile \&I'}
  	// \xspace\xspace\xspace   --- since \tcode{D} has the above form: \tcode{'* cv D1'} where \tcode{'D1 = *volatile \&R1'} [Replacing \tcode{'I'} with \tcode{'R1'} to track recursion]
  	// \xspace\xspace\xspace   --- lets call the full type of the identifier \tcode{R1} in the intermediate hypothetical declaration \tcode{T D1} 
  	// \xspace\xspace\xspace\xspace\xspace\xspace  (i.e. \tcode{'const int *volatile \&R1'}) as \tcode{U1} then \tcode{U1} can be decomposed into \tcode{X1} \tcode{T} (\tcode{X1} is the 'derived-declarator-type-list')
  	// \xspace\xspace\xspace   --- the type of \tcode{I} is \tcode{'X1} const pointer to \tcode{T'}, so now recurse to determine \tcode{'X1'} since we know \tcode{T = 'const int'}
  	// \xspace\xspace\xspace   --- [NOTE: from last bullet in step 2 below, our recursive step returns \tcode{X1} = 'reference to volatile pointer to', 
  	// \xspace\xspace\xspace\xspace\xspace\xspace so plug it in above]
  	// \xspace\xspace\xspace   --- therefore, the type of \tcode{I} is = 'reference to volatile pointer to const pointer to \tcode{T}' and since \tcode{T} = \tcode{const int}, 
  	// \xspace\xspace\xspace\xspace\xspace\xspace therefore the type of \tcode{I} = 'reference to volatile pointer to const pointer to \tcode{const int}'
  	...
  	// \xspace\xspace\xspace   --- so now \tcode{T = 'const int'} and the new \tcode{D = '*volatile \&R1'}
  	// \xspace\xspace\xspace   ---  since \tcode{D} has the above form: \tcode{'* cv D2'} where \tcode{'D2 = \&R2'} [Replacing \tcode{'R1'} with \tcode{'R2'} to track recursion]
  	// \xspace\xspace\xspace --- lets call the full type of the identifier \tcode{R2} in the intermediate hypothetical declaration \tcode{T D2} 
  	// \xspace\xspace\xspace\xspace\xspace\xspace (i.e. \tcode{const int \&R2}) as \tcode{U2} then \tcode{U2} can be decomposed into \tcode{X2} \tcode{T} (\tcode{X2} is the 'derived-declarator-type-list')
  	// \xspace\xspace\xspace   --- the type of \tcode{R1} is \tcode{'X2} volatile pointer to \tcode{T'}, so now recurse to determine \tcode{'X2'} since we know \tcode{T = 'const int'}
  	// \xspace\xspace\xspace   --- [NOTE: from last bullet in step 3 below, our recursive step returns \tcode{X2} = 'reference to', so plug it back in above]
  	// \xspace\xspace\xspace   --- therefore, \tcode{U1} = 'reference to volatile pointer to \tcode{T}' and since \tcode{U1} = \tcode{X1 T}, 
  	// \xspace\xspace\xspace\xspace\xspace\xspace therefore \tcode{X1} = 'reference to volatile pointer to'
  	...
  	// \xspace\xspace\xspace   --- so now \tcode{T = 'const int'} and the new \tcode{D = '\&R1'}
  	// \xspace\xspace\xspace   --- since \tcode{D} has the form: \tcode{'\& cv D3'} where \tcode{'D3 = R3'} [Replacing \tcode{'R2'} with \tcode{'R3'} to track recursion]
  	// \xspace\xspace\xspace --- lets call the full type of the identifier \tcode{R3} in the intermediate hypothetical declaration \tcode{T D3} (i.e. \tcode{const int R3})
  	// \xspace\xspace\xspace\xspace\xspace\xspace as \tcode{U3} then \tcode{U3} can be decomposed into \tcode{X3} \tcode{T} (\tcode{X3} is the 'derived-declarator-type-list') and in this case \tcode{X3} is empty.
  	// \xspace\xspace\xspace   --- the type of \tcode{R2} is \tcode{'X3} reference to \tcode{T'}, and since \tcode{X3} is empty, the type is 'reference to \tcode{T}'
  	// \xspace\xspace\xspace   --- therefore, \tcode{U2} = 'reference to \tcode{T}' and since \tcode{U2} = \tcode{X2 T}, therefore \tcode{X2} = 'reference to'
  \end{codeblock}
\end{fvnote}
