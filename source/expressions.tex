...
On \uline{PRECEDENCE} and \uline{ASSOCIATIVITY}\newline
	...
	\mbox{\xspace \xspace multiplicative-expression}\newline
	\mbox{\xspace\xspace additive-expression + multiplicative-expression}\newline
	\mbox{\xspace\xspace additive-expression - multiplicative-expression}\newline\newline}
...
\end{example}
\fvmargin{
\href{http://lists.isocpp.org/core/2017/10/3129.php}{Richard and Daveed CWG}\newline\newline
> Daveed: What happens if the lambda Doesn't have a capture default?  E.g.:\newline
\tcode{
        void g(int x) \{ \newline
          \mbox{\xspace\xspace []\{ } \newline
          \mbox{\xspace\xspace\xspace\xspace  decltype((x)) y = 1;  // Type of y?} \newline
          \mbox{\xspace\xspace\}();}\newline
        \}\newline
}
Clang changes the type to int\& (from const int\&).  \newline
> Richard:\newline
IIUC, the point of this rule is to deal with the cases where 'x' is captured, but we don't find out until later in the lambda's body. It would make sense to me for this special case to only apply when the lambda has an [=] capture-default. (Note that the wording also doesn't work if the lambda has a [\&] capture-default, since there is no such corresponding data member in that case.) Put another way, decltype((x)) should give the type that the expression 'x' would have if it appeared in the lambda-body outside of an unevaluated operand.\newline
Given that model, the type of y would be 'int\&' unless the lambda has a capture-default of [=], in which case the type would be 'const int\&'. As evidence that this is a somewhat-consistent model, consider:\newline
\tcode{int g;\newline
void f() \{\newline
  \mbox{\xspace\xspace int \&v = g;}\newline
  \mbox{\xspace\xspace[?]\{ decltype(auto) r = (v); \};}\newline
\} \newline
}\newline
* If the capture list has a [=] capture-default and doesn't mention v, or explicitly captures v by copy, the type of 'r' is 'const int \&', and the reference binds to the member of the closure type.\newline
 * If the capture list has any other capture list, the type of 'r' is 'int \&', and the reference binds to the global variable 'g'.\newline
[=] fundamentally changes what 'x' would mean within the lambda, to make it refer to a different entity than it would with []. It seems reasonable to me that it would also affect the type as determined by decltype.
}
...
requirements as detailed in this subclause 

\fvmargin{
  Certain 'grammar productions' require \grammarterm{constant-expressions}: \newline%
  \mbox{ \xspace\xspace 1. \grammarterm{noptr-new-declarator [ constant-expression ]}}\newline%
  \mbox{ \xspace\xspace\xspace\xspace ----- e.g.  \tcode{new int[n][10]}}\newline%
  \mbox{ \xspace\xspace 2. \tcode{case} \grammarterm{constant-expression} : }\newline%
  \mbox{ \xspace\xspace 3. \tcode{static_assert} ( \grammarterm{constant-expression} ) ; }\newline%
  \mbox{ \xspace\xspace 4. \tcode{static_assert} ( \grammarterm{constant-expression, string-literal} ) ; }\newline%
  \mbox{ \xspace\xspace 5. \grammarterm{enumerator = constant-expression} }\newline%
  \mbox{ \xspace\xspace 6. \tcode{alignas}(\grammarterm{constant-expression ...opt}) }\newline%
  \mbox{ \xspace\xspace 7. \grammarterm{noptr-declarator [ constant-expressionopt ]}}\newline%
  \mbox{ \xspace\xspace\xspace\xspace ----- e.g.  \tcode{int arr[10]}}\newline%  
  \mbox{ \xspace\xspace\xspace\xspace ----- e.g.  \tcode{void f(int (*)[10])}}\newline%  
  \mbox{ \xspace\xspace 8. bit field => \grammarterm{identifier opt : constant-expression}}\newline%
  \mbox{ \xspace\xspace\xspace\xspace ----- e.g.  \tcode{struct \{ int : 10; \};}}\newline%  
  \mbox{ \xspace\xspace 9. \grammarterm{template-argument: constant-expression }}\newline%
  \mbox{ \xspace\xspace\xspace\xspace ----- e.g.  \tcode{array<10> a;}}\newline%  
  \mbox{ \xspace\xspace 10. \tcode{noexcept} ( \grammarterm{constant-expression} ) }\newline%
}
...
assuming that copy elision is performed \begin{fvnote}Copy elision is mandatory for constant expressions \fvcwg{2022} \end{fvnote}, are called 
...
\begin{fvnote} The reasons for resistance to clearly stating that constant expression evaluation occurs during  translation phase 7 and 8 is somewhat described in the following thread \href{http://lists.isocpp.org/core/2016/07/0741.php}{cwg-discussion-741} 
\end{fvnote}
