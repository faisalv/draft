...
\end{footnote}
\fvmargin{
	On \uline{PRECEDENCE} and \uline{ASSOCIATIVITY}\newline%
	\href{https://stackoverflow.com/questions/20767745/who-defines-c-operator-precedence-and-associativity}{StackOverFlow answer}\newline%
	\textit{additive-expression:\newline
		\mbox{\space\space multiplicative-expression}\newline
		\mbox{\space\space additive-expression + multiplicative-expression}\newline
		\mbox{\space\space additive-expression - multiplicative-expression}\newline\newline}
...
\fvmargin{ See Walter's Paper regarding contextual implict conversions: \fvpaper{n3323}\newline
	\fvmarginspace{--}--- delete operand: \newline
	\fvmarginspace{--------} class => pointer to object\newline
	\fvmarginspace{--}--- constant-expression: \newline
	\fvmarginspace{--------}literal class => integral or unscoped enum\newline
	\fvmarginspace{--}--- switch condition: \newline
	\fvmarginspace{--------}class => integral or enum\newline
}
...
\end{note}
\fvmargin{\href{https://en.cppreference.com/w/cpp/language/implicit_conversion}{cppref} Value transformations are conversions that change the value category of an expression such as when an expression appears as an operand of an operator that expects an expression of a different value category. }
...
cv-qualified types.
\end{footnote}
\fvmargin{This conversion models the act of reading a value from a memory location into a CPU register.}
\pnum
...
\fvmargin{
	See \fvpaper{p0135r0}\newline
	See \fvgit{611f4c675e2160463cb1fbf71ab87bd67c9102eb}
}
\pnum
...
\begin{fvfootnote}
	\href{https://stackoverflow.com/questions/56733770/multiple-cv-decompositions-of-a-type}{SO 1}, 
	\href{https://stackoverflow.com/questions/54886467/ambiguity-in-qualification-conversion}{SO 2}, 
	\href{https://blog.panicsoftware.com/qualification-conversion/}{Blog Post}, \fvcwg{330}, \fvpaper{n4261}, \href{https://listarchives.isocpp.org/cgi-bin/wg21/message?wg=core&msg=26242}{cwg-reflector},
	\href{https://listarchives.isocpp.org/cgi-bin/wg21/message?wg=core&msg=25976}{cwg-reflector},
	\href{https://listarchives.isocpp.org/cgi-bin/wg21/message?wg=core&msg=25423}{cwg-reflector}
	To obtain the cv-decomposition of type T1 (a type can have multiple cv-decompositions if it has multiple ptr-operators), \uline{reorder} the type so as to have each cv-qualifier be to the right of \tcode{'U'} or the corresponding '*'.  Then using the ptr-operators as separators, separate the cv-specifiers into a sequence of (cv, ptr) pairs, numbered from high to low as you move further away from \tcode{U}. As an example consider the three cv-decompositions of \tcode{int const **}\newline
	\fvfootnotespace{----}1) \tcode{U=int const** cv0=()} N=0\newline
	\fvfootnotespace{----}2) \tcode{U=int const*\fvfootnotespace{*}  cv0=(), P0=(*), cv1=()} N=1  \newline
	\fvfootnotespace{----}3) \tcode{U=int \fvfootnotespace{const**}  cv0=(), P0=(*), cv1=(), P1=(*), cv2=(const) }  N=2 : \tcode{int const2 **}\newline\newline	
    Examples:\newline
	\fvfootnotespace{----}\tcode{const int *const (X::*const *const X::*pmi)[]  = (int* (X::** X::*)[10]) 0;}\newline
	\fvfootnotespace{----}\tcode{int *(X::*const *const X::*pmi2)[]  = (int* (X::** X::*)[10]) 0;}\newline
	\fvfootnotespace{----}\tcode{int volatile *const *pi = (int**)0;}\newline
	\fvfootnotespace{----}\tcode{int **volatile *const *pi2 = (int **volatile **)0;}\newline	
\end{fvfootnote}
\fvmargin{	
	To \textbf{convert T1 into T2}:\newline
	1) T1 and T2 shall be \textbf{similar} (i.e. types are the same ignoring cv-qualifiers) \newline 
	\fvmarginspace{---} i.e \tcode{int**} and \tcode{int *const*} are similar.\newline 
	2) The cv-qualifiers corresponding to each ptr-operator (numbered in \textbf{descending} order from 'U') must be the same.\newline 
	\fvmarginspace{---} i.e. \tcode{int const2 *const1 *const0}\newline
	3) As we descend from \tcode{U} if there is a 'k' where qualifiers do not match between \tcode{T1 \& T2}, then all qualifiers from k...1 in T2 must be cv-qualified (can ignore cv0 '\tcode{int i = (int const) 0; }' is OK):\newline
	\fvmarginspace{---}\tcode{struct X;}\newline
	\fvmarginspace{---}\tcode{using U = const char (X::*())[10];}\newline
	\fvmarginspace{---}\tcode{U const *const *const * p1 = (U ***const)0;}\newline
	\fvmarginspace{---}\tcode{U **const *const * p2 = (U **const **)0;}\newline

	\textbf{\tcode{volatile}} must match exactly if in T1 - except for cv0\newline\newline
	\textbf{\tcode{const}} must be added for every lower cvK except cv0:\newline
}
...
\fvmargin{\newline
\tcode{
  \fvmarginspace{--} struct A \{ operator int*() \{ return 0; \} \};\newline
  \fvmarginspace{--} delete A(); } // OK!\newline
}
...
\fvmargin{
Computing modulus for powers of two  is much more efficient with bit-arithmetic on a binary computer. The term \tcode{(a \% m)} is zero, so we just take the modulus by examining the least significant bits of the binary representation:\newline\newline 
\tcode{\fvmarginspace{--} a \% 2\textsuperscript{i} = a \& (2\textsuperscript{i} – 1)\newline	
\fvmarginspace{--} a \% 2 => a \& 1\newline
\fvmarginspace{--} a \% 4 => a \& 3\newline
\fvmarginspace{--} a \% 8 => a \& 7\newline}\newline
\href{http://homepage.cs.uiowa.edu/~jones/bcd/mod.shtml}{Modulo sans division}, \href{https://stackoverflow.com/questions/3072665/bitwise-and-in-place-of-modulus-operator}{SO bitwise in-place of mod}
}
...
\pnum
\fvmargin{The reasons for resistance to clearly stating that constant expression evaluation occurs during  translation phase 7 and 8 is somewhat described in the following thread: \href{http://lists.isocpp.org/core/2016/07/0741.php}{cwg-discussion-741}}
...
\begin{note}
\fv{Constant expressions can be evaluated
during translation.}
\end{note}
...
\fvmargin{
	Certain 'grammar productions' require \grammarterm{constant-expressions}: \newline%
	\mbox{ \space\space 1. \grammarterm{noptr-new-declarator [ constant-expression ]}}\newline%
	\mbox{ \space\space\space\space ----- e.g.  \tcode{new int[n][10]}}\newline%
	\mbox{ \space\space 2. \tcode{case} \grammarterm{constant-expression} : }\newline%
	\mbox{ \space\space 3. \tcode{static_assert} ( \grammarterm{constant-expression} ) ; }\newline%
	\mbox{ \space\space 4. \tcode{static_assert} ( \grammarterm{constant-expression, string-literal} ) ; }\newline%
	\mbox{ \space\space 5. \grammarterm{enumerator = constant-expression} }\newline%
	\mbox{ \space\space 6. \tcode{alignas}(\grammarterm{constant-expression ...opt}) }\newline%
	\mbox{ \space\space 7. \grammarterm{noptr-declarator [ constant-expressionopt ]}}\newline%
	\mbox{ \space\space\space\space ----- e.g.  \tcode{int arr[10]}}\newline%  
	\mbox{ \space\space\space\space ----- e.g.  \tcode{void f(int (*)[10])}}\newline%  
	\mbox{ \space\space 8. bit field => \grammarterm{identifier opt : constant-expression}}\newline%
	\mbox{ \space\space\space\space ----- e.g.  \tcode{struct \{ int : 10; \};}}\newline%  
	\mbox{ \space\space 9. \grammarterm{template-argument: constant-expression }}\newline%
	\mbox{ \space\space\space\space ----- e.g.  \tcode{array<10> a;}}\newline%  
	\mbox{ \space\space 10. \tcode{noexcept} ( \grammarterm{constant-expression} ) }\newline%
}
...
\begin{fvnote}
NOT SURE this is still relevant on 6-15-24
\begin{codeblock}
  struct X { constexpr int f() const { return 0; } };
  constexpr int foo(const X& r) { r; return r.f(); }
  int main() {
    X x;
	constexpr int I = foo(x);
  }
\end{codeblock}
In the above code, in the evaluation of the id-expression \tcode{'r'} which refers to a variable (a parameter is a variable) of reference type, \tcode{r's} lifetime begins within the	evaluation of \tcode{'e'} - and so the above is OK in a constant expression.
\end{fvnote}
