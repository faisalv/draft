...
\marginfv{
 ...
 \begin{itemize}
 	...
 	\item For function like macros, the arguments are first expanded prior to substitution within the replacement list (unless the argument has either \tcode{\#} or \tcode{\#\#} applied to it.)
 	\item Macro arguments are optional, but the commas for non-ellipsis parameters are not optional i.e\newline%
 	\newline%
 	...
 	\item Each Macro has two flags associated with it DISABLED, IS_FUNCTION
 \end{itemize}
}
