...
\fvmargin{
	\href{https://gcc.gnu.org/onlinedocs/gcc-6.3.0/cpp/}{GCC Preprocessor Manual}\newline
	Preprocessing directives are lines in your program that start with '\tcode{\#}'. Whitespace is allowed before and after the '\tcode{\#}'. \newline
	\newline
	\underline{Macro Replacement Process Sketch}\newline
	\begin{itemize}
		\item When scanning/lexing, if an identifier that has been defined as a macro is found, it is replaced with its replacement-list (phase 4)
		\item Argument Expansion for function like macros: the arguments are first expanded prior to substitution within the replacement list (unless the argument has either \tcode{\#} or \tcode{\#\#} applied to it.).  
		\begin{itemize}
			\item A macro-id for a function-like macro is not disabled until after argument expansion so for e.g. this is OK (note: \tcode{x,y} do not form two arguments for outer invocation of \tcode{A}):\newline
			\newline
			\tcode{\#define A(x,y) x,y\newline
				\mbox{ \space\space A(A(1,2),3) ==> 1,2,3}\newline
			}
			\item if during argument expansion a macro-id is recursively expanded it is disabled, then when examined in the context of the replacement list it is still not expanded. Note, since A is disabled in the invocation of B, it is legal to end up with A() since the preprocessor won't try to macro-expand it\newline
			\newline
			\tcode{\#define A(x,y) A\newline
				\#define B(b) b\newline
				\mbox{ \space\space A() ==> ERROR!}\newline
				\mbox{ \space\space B(A(1,2))() ==> A()}
			}
		\end{itemize}
		\item Macro arguments are optional (replaced by 'placemarker' tokens), but the commas for non-ellipsis parameters are not optional i.e\newline
		\newline
		\tcode{\#define X(a,b) \#a,\#b\newline
			\mbox{ \space\space X(,) ==> "",""}\newline
			\mbox{ \space\space X() ==> ERROR!}\newline
			\mbox{ \space\space X(u,) ==> "u",""}\newline
			\mbox{ \space\space X(,v) ==> "","v"}\newline}
		\item When a macro is replaced, the context of that macro is popped off (since function-like macros are not invoked until the left-paren which is seen following context-pop-off, NO recursively disabled macros below ):\newline
		\newline
		\tcode{\#define A(x) a x\newline
			\#define B(x) b x\newline
			\mbox{\space\space A(B)(A)(B)(abc) ==> a b a b abc}
			\newline\newline
			\#define A2(x) x <a2> A1\newline
			\#define A1(x) x <a1> A2\newline
			\mbox{\space\space A1(2)(3)(4) ==> 2 <a1> 3 <a2> 4 <a1> A2}
			\mbox{\space\space A2(2)(3)(4) ==> 2 <a2> 3 <a1> 4 <a2> A1}
		}
	\end{itemize}
}
...
\fvmargin{The C standard states that, after any parameters have been replaced with their possibly-expanded arguments, the replacement list is scanned for nested macros. Further, any identifiers in the replacement list that are not expanded during this scan are never again eligible for expansion in the future, if the reason they were not expanded is that the macro in question was disabled.\newline%
	...
	\newline%
	\tcode{\#define foo(x) bar x\newline%
		foo(foo) (2)}\newline%
	\newline%
	which fully expands to \tcode{bar foo (2)}. During pre-expansion of the argument, \tcode{foo} does not expand even though the macro is enabled, since it has no following parenthesis [pre-expansion of an argument only uses tokens from that argument; it cannot take tokens from whatever follows the macro invocation]. This still leaves the argument token \tcode{foo} eligible for future expansion. Then, when re-scanning after argument replacement, the token \tcode{foo} is rejected for expansion, and marked ineligible for future expansion, since the macro is now disabled. It is disabled because the replacement list \tcode{bar foo} of the macro is still on the context stack.\newline%
	\newline%
...
\end{bnf}
\fvmargin{\newline%
	\href{http://www.boost.org/doc/libs/release/libs/wave/doc/macro_expansion_process.html}{1. Wave Mensonides Algorithm} \newline%
	\href{https://gcc.gnu.org/onlinedocs/gcc-6.3.0/cpp/Argument-Prescan.html}{2. GCC Argument Prescan} \newline%
	\href{https://gcc.gnu.org/onlinedocs/cppinternals/Macro-Expansion.html}{3. GCC Macro Expansion} \newline%
	}
\pnum
...
\fvmargin{
    Only \tcode{\#\#} within a replacement list leads to concatenation.  Below the argument is not within a replacement list and does not lead to concat during argument substitution!
	\newline
	\tcode{\#define D(x) x
		\newline\newline
		D(A \#\# B)  ==>  A \#\# B [Not 'AB']
...
\fvmargin{
	\newline%
	Notice the cool manipulation involved below:\newline%
	\tcode{
		\#define A(x) x \newline
		\#define B(x) x \newline
		A( B )( () ) // expands to: () \newline
		...
		\newline
		\newline
		NOTE: Keep in mind with the below example that for a function-like macro invocation, unless the token following the name is actually an ( - as opposed to something that expands to an '(' no expansion takes place during that scan.
		The following example illustrates:\newline
		- EXP1: no argument expansion with concatenation\newline
		- EXP2: argument expansion\newline 
		- EXP3: \newline
		\#define LPAREN (  \newline
		\#define W() X LPAREN ) \newline
		\#define X() Y LPAREN ) \newline
		...
		\newline
		\#define EXP1(a, b)  a \#\# b \newline
		\#define EXP2(a)  EXP1(a,) \newline
		\#define EXP3(a)  EXP2(a) \newline
		\newline
		// expands only during final rescanning\newline
		EXP1(W(),)    ==> X() \newline\newline
		// expands once during argument substitution\newline
		// then during rescanning \newline
		EXP2(W())     ==> Y() \newline\newline
		// expands twice during argument substitution\newline
		// then once during rescanning\newline
		EXP3(W())     ==> Z() \newline
	} 
}
