...
\fvmargin{ As to why constexpr is not optional on function definitions see this thread:
\href{http://lists.isocpp.org/core/2016/07/0737.php}{cwg-discussion0737}\newline
I believe the arguments for making 'constexpr' optional are:\newline
- 'constexpr' on a function definition might imply that a function
is usable in a constant-expression - but that's not true - so it can
be confusing.\newline
- we can get excellent diagnostics for why a function is not usable
in a constant-expression when used, so let's just rely on that.\newline
- we open more functions/algorithsm up to constant-expression
evaluation - regardless of whether authors are seasoned enough to
think about whether their functions should be marked constexpr.\newline
- library folks can just document that their functions are supposed
to be usable in a constant expression (perhaps through an attribute -
which wouldn't affect semantics)\newline
- we can reduce pseudo-checking functions for 'constexpr-ness',
assuming folks don't use it on every function that they think might be
usable in a constant expression.\newline
- what else?\newline
Arguments against making it optional are:\newline
- we signify the intent (contract) that the function was designed to
be usable in a constant expression (but this could be done through
documentation/attributes?) and so use in a constant-expression is
future-proof?\newline
- we might trigger more template instantiations for templates that
don't even meet the minimal requirements.\newline
- what else?\newline
> If the authors did not intend for the function to be usable in a constant\newline
> expression (but it happens to be usable currently), users may come to depend\newline
> (or accidentally depend) on the usability of the function in constant\newline
> expressions. I can just imagine someone screaming bloody murder because the\newline
> initialization of their non-locals gets "broken" when some random function\newline
> is refactored so that it calls something from a separately-compiled library.\newline
>\newline
Agreed - this is a compelling risk. Not sure how prevalent it truly
would be, but I admit it doesn't seem that unusual.  I suppose this
concern in itself is enough to kill any thoughts of making 'constexpr'
optional on a function?
}
...
\begin{fvnote}
	\textit{derived-declarator-type-list} is the assumed known variable that is used to recursively determine the description of a type through syntactic pattern-matching, decomposition and recomposition. In spite of the italics it is NOT a grammar term. It is used as follows: Say one wants to determine the full type of a variable declared with a complicated combination of declarator-operators (i.e. *, \&, (), []), then assuming that we know how the inner declarators have affected the type (since we start calculation using the inner most declarator-operators, i.e. those closest to the identifier) --- i.e. assume the resulting type using \tcode{'N'} inner declarators is {\color{addclr}{'\textit{derived-declarator-type-list} \tcode{T}'}}, then using the current declarator ('*') we can determine that the current type (i.e. then for \tcode{'N1'} is {\color{addclr}'\textit{derived-declarator-type-list} {\color{orange} pointer to} \tcode{T}'}).  \term{derived-declarator-type-list} consists of some valid concatenated sequence of the following phrases [that ends with \tcode{T}]: \begin{enumerate}
		\setlength\itemsep{0mm}
		\item \textit{cv-qualifier-seq} pointer to
		\item \textit{cv-qualifier-seq} reference to
		\item \textit{cv-qualifier-seq} pointer to member of class nested-name-specifier of type
		\item array of \tcode{N} \textit{cv-qualifier-seq}
		\item array of unknown bound of
		\item \opt{\tcode{noexcept}} function of (\grammarterm{parameter-declaration-clause} )
		\grammarterm{\opt{cv-qualifier-seq}} \grammarterm{\opt{ref-qualifier}} returning'
	\end{enumerate}
	For e.g. \begin{codeblock}
  const int *const  *volatile &I
  // 1) start with first decomposing the declaration of \tcode{I} into \tcode{'T D'} where:
  // \space\space\space   --- \tcode{T = 'const int'} and \tcode{D = '*const *volatile \&I'}
  // \space\space\space   --- since \tcode{D} has the above form: \tcode{'* cv D1'} where \tcode{'D1 = *volatile \&R1'} [Replacing \tcode{'I'} with \tcode{'R1'} to track recursion]
  // \space\space\space   --- lets call the full type of the identifier \tcode{R1} in the intermediate hypothetical declaration \tcode{T D1} 
  // \space\space\space\space\space\space  (i.e. \tcode{'const int *volatile \&R1'}) as \tcode{U1} then \tcode{U1} can be decomposed into \tcode{X1} \tcode{T} (\tcode{X1} is the 'derived-declarator-type-list')
  // \space\space\space   --- the type of \tcode{I} is \tcode{'X1} const pointer to \tcode{T'}, so now recurse to determine \tcode{'X1'} since we know \tcode{T = 'const int'}
  // \space\space\space   --- [NOTE: from last bullet in step 2 below, our recursive step returns \tcode{X1} = 'reference to volatile pointer to', 
  // \space\space\space\space\space\space so plug it in above]
  // \space\space\space   --- therefore, the type of \tcode{I} is = 'reference to volatile pointer to const pointer to \tcode{T}' and since \tcode{T} = \tcode{const int}, 
  // \space\space\space\space\space\space therefore the type of \tcode{I} = 'reference to volatile pointer to const pointer to \tcode{const int}'
		
		
  // 2) To determine \tcode{X1}, we need to determine \tcode{U1} (i.e. \tcode{X1 T}) which is the full type of \tcode{R1} (\tcode{T D1}: \tcode{const int *volatile \&R1})
  // \space\space\space   --- so now \tcode{T = 'const int'} and the new \tcode{D = '*volatile \&R1'}
  // \space\space\space   ---  since \tcode{D} has the above form: \tcode{'* cv D2'} where \tcode{'D2 = \&R2'} [Replacing \tcode{'R1'} with \tcode{'R2'} to track recursion]
  // \space\space\space --- lets call the full type of the identifier \tcode{R2} in the intermediate hypothetical declaration \tcode{T D2} 
  // \space\space\space\space\space\space (i.e. \tcode{const int \&R2}) as \tcode{U2} then \tcode{U2} can be decomposed into \tcode{X2} \tcode{T} (\tcode{X2} is the 'derived-declarator-type-list')
  // \space\space\space   --- the type of \tcode{R1} is \tcode{'X2} volatile pointer to \tcode{T'}, so now recurse to determine \tcode{'X2'} since we know \tcode{T = 'const int'}
  // \space\space\space   --- [NOTE: from last bullet in step 3 below, our recursive step returns \tcode{X2} = 'reference to', so plug it back in above]
  // \space\space\space   --- therefore, \tcode{U1} = 'reference to volatile pointer to \tcode{T}' and since \tcode{U1} = \tcode{X1 T}, 
  // \space\space\space\space\space\space therefore \tcode{X1} = 'reference to volatile pointer to'
		
  // 3) To determine \tcode{X2}, we need to determine \tcode{U2} (i.e. \tcode{X2 T}) which is the full type of \tcode{R2} (\tcode{T D2}: \tcode{const int \&R2})
  // \space\space\space   --- so now \tcode{T = 'const int'} and the new \tcode{D = '\&R1'}
  // \space\space\space   --- since \tcode{D} has the form: \tcode{'\& cv D3'} where \tcode{'D3 = R3'} [Replacing \tcode{'R2'} with \tcode{'R3'} to track recursion]
  // \space\space\space --- lets call the full type of the identifier \tcode{R3} in the intermediate hypothetical declaration \tcode{T D3} (i.e. \tcode{const int R3})
  // \space\space\space\space\space\space as \tcode{U3} then \tcode{U3} can be decomposed into \tcode{X3} \tcode{T} (\tcode{X3} is the 'derived-declarator-type-list') and in this case \tcode{X3} is empty.
  // \space\space\space   --- the type of \tcode{R2} is \tcode{'X3} reference to \tcode{T'}, and since \tcode{X3} is empty, the type is 'reference to \tcode{T}'
  // \space\space\space   --- therefore, \tcode{U2} = 'reference to \tcode{T}' and since \tcode{U2} = \tcode{X2 T}, therefore \tcode{X2} = 'reference to'
\end{codeblock}
\end{fvnote}
\fvmargin{In regards to: 'derived-declarator-type-list' \newline\newline
	\href{https://groups.google.com/d/topic/comp.std.c/gIEwLcEhCHE/discussion}{1990 Doug Gwyn} "The examples in
	...
	\href{http://stackoverflow.com/questions/13808932/what-are-declarations-and-declarators-and-how-are-their-types-interpreted-by-the}{1) Must Read stack-overflow}\newline
	\href{http://stackoverflow.com/questions/13779273/in-the-standard-what-is-derived-declarator-type}{2) Optional stackoverflow-discussion}
}
The
\grammarterm{cv-qualifier}{s}
...
\fvmargin{The 'naming class' is defined in \ref{class.access.base}/5. It is where name lookup starts.  The naming class is explicit in a qualified-id such as  in \tcode{d->A::x()} it is \tcode{'A'} or implicit when it refers to the type of the object expression such as in \tcode{d->x()} it is \tcode{'D'}.   The other piece to this is that in \tcode{E1.E2} there are two conditions that must hold:
	\begin{enumerate}
		\item The declaring class of the member (i.e the class of which it is directly a member) must be unambiguously convertible to the naming class of E2 \ref{expr.ref}/5
		\item The class of the object expression must be implicitly \ref{conv.ptr}/3 convertible (i.e unambiguously) to the type of the naming class \ref{class.access.base}/6. 
	\end{enumerate}
}
...
void f(D* d) {
  // Below, the 'naming-class' refers to the class in which lookup for \tcode{x} begins, 
  // the 'declaring-class' is the class that has the selected \tcode{x} as its direct member
  // (i.e ignoring using-declarations), and the 'object-class' is the class of the 
  // object expression.
	
  d->x();       // ambiguous: declaring-class \tcode{A} is an ambiguous base of the implicit
                // naming-class \tcode{D}
  ...
  d->C::x();    // OK: declaring-class \tcode{A} is an unambiguous base of the naming-class
                // \tcode{C}, which is an unambigous base of the object-class \tcode{D}            
  d->C::x(3);   // OK: naming-class and declaring-class \tcode{C} is an umambiguous base of
                // the object-class \tcode{D}  
  d->B::x();    // ambiguous: declaring-class \tcode{A} is an ambiguous base of the naming-class \tcode{B}
  d->A1::x();   // OK: declaring-class \tcode{A} is an unambiguous base of the naming-class
                // \tcode{A1} which is an unambiguous base of the object-class \tcode{D}
  d->B::y();    // ambiguous: member lookup ambiguity when merging lookup sets of 
                // \tcode{B}'s direct base subobjects 
  d->B::z();    // OK: using-declaration resolves member lookup ambiguity in favor of
                // \tcode{A1::z} which hides \tcode{A::z} (same signature); and, 
                // declaring-class \tcode{A1} is an unambiguous base of naming-class \tcode{B} 
                // which is an unambiguous base of object-class \tcode{D}
  d->z();       // ambiguous: member name lookup ambiguity when merging lookup 
                // sets from \tcode{B} and \tcode{C} subobjects
}
