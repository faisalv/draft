...
\fvmargin{
	\href{https://www.iso.org/sites/directives/current/part2/index.xhtml\#_idTextAnchor072}{ISO Verbal Forms}, \fvpaper{n4879}, Related editorial commits: \fvgit{431818492f83ca3c3a188906ad562ec24d9aa664},  \fvgit{84087b3d3f0f60a2878a89810a1c4d0e354722d9},  \fvgit{cb07613ed7c6cfa19a5ce024a5a8ed1c6d9bf869},  \fvgit{64b5913f6b3cefc42a98f86e68d88e9cef752c82},  \fvgit{e5455e3bb745c82b98569d692265c6ab559eb00c}, 
	\fvgit{84f8f1aaa3fbd535dd02af3107d9271ac705c330}, 
	\fvgit{44f4e7926dde3b323427dae876be6b2dfbca5f05}
	\newline\newline
	\underline{Requirements}: \textbf{shall, shall not} \newline 
	--  Do not use "must" as an alternative for "shall". This avoids confusion between the requirements of a document and external constraints\newline
	--  Do not use "may not" instead of "shall not" to express a prohibition.\newline
	\newline  
	\underline{Recommendation}: \textbf{should, should not} \newline
	\newline 
	\underline{Permission}: \textbf{may, may not}  \newline
	...
	\newline
	\underline{Possibility and capability}: \textbf{can, cannot}\newline
	-- Do not use "may" instead of "can" in this context.\newline
	-- "May" signifies permission expressed by the document, whereas "can" refers to the ability of a user of the document or to a possibility open to him/her.\newline
	-- For e.g: \newline
	---- In some cases the statement needs to be examined (NOT: the statement might need to be examined)\newline
	---- It is possible for a class template (NOT: a template might)\newline
	---- There is no context in which they \textbf{WOULD} be used (NOT: they could be used)\newline
	\newline 
	\underline{External constraints/information}: \textbf{must}\newline
	-- Do not use "must" as an alternative for "shall". This avoids confusion between the requirements of a document and external constraints\newline
	-- Disambiguation rules \textbf{are} applied (NOT: must be applied)
}
...
``undefined behavior''.
\begin{fvfootnote}
	\textbf{Undefined Behavior}: UB doesn't cause issues unless a specific execution triggers it - but it reaches backwards, and could impact behavior of code that appears to be invoked before it. (likely to allow code rearrangements and optimizations):  \tcode{*p}
	//leads to UB if p=0 \newline
	\textbf{Ill Formed NDR (leads to UB if not diagnosed)}: IFNDR should be diagnosed during translation, but may not be able to due to various technical or theoretical limitations. E.g. the ODR would require the implementation to collect all definitions of an entity and compare them; Some NDR things are even computationally infeasible. 
	Undefined behavior arises when the implementation doesn't immediately diagnose this stuff.
	
\end{fvfootnote}
\fvmargin{
	\textbf{\underline{UB vs IFNDR}} 	\href{https://lists.isocpp.org/ext/2018/06/4627.php}{reflector},
	\href{https://stackoverflow.com/questions/22180312/difference-between-undefined-behavior-and-ill-formed-no-diagnostic-message-requ}{SO}
	\href{https://en.cppreference.com/w/cpp/language/ub}{cppref}	
	\newline\textbf{UB only affects those program executions that would execute the UB.  The rationale for choosing UB over ill-formed NDR is you get the UB only if you trigger it, as opposed to the mere presence of an ill-formed construct turning your whole program into UB if you run it, regardless of whether you trigger the side effects or not.}
	\newline\newline
	\textbf{Examples of UB:}\newline
	-- signed overflow: \tcode{void f(int x) \{ return x + 1; \}}\newline
	-- access out of bounds \newline
	-- read an uninitialized scalar \newline
	-- null pointer dereference \newline
	-- access to pointer passed to realloc \newline
	-- infinite loop without side-effects \newline
	-- invalid scalar values assigned via reinterpret-cast\newline
	\textbf{Examples of IFNDR:}\newline
	-- [lex.comment] vertical-tab/ff characters in // comments followed by non-whitespace\newline
	-- [lex.name] identifiers with __ or _[A-Z] in any scope\newline
	-- [lex.name] identifiers with _[a-zA-Z1-0] in global scope\newline
	-- [basic.link] two declarations of an entity attached to different module and neither is reachable from the other.\newline
	-- [basic.link] array object declarations that differ by presence or absence of a major array bound, and neither is reachable from the other.\newline
	-- [stmt.ambig] lookup finds name in template argument bound to part of declaration being parsed \tcode{int B(C<decltype(B)>())}%
}
