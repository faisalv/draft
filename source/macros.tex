...
%%%%%%%%%%%%%%%% BEGIN FV Annotation Macros %%%%%%%%%%%%%%%%%%%%%%%%

\definecolor{orange}{rgb}{0.9,0.4,0}
\definecolor{indigo}{rgb}{0,0.4,1}
\definecolor{darkpurple}{rgb}{0.4,0,1}

\newcounter{fvnum} %every annotation is given a number

%every annotation has an id.
\newcommand{\fvid}{\tiny{\ensuremath{^{.fv\arabic{fvnum}}}}}
% Do not embed the stepcounter, since it leads to erroneous calculations.
%\newcommand{\fvid}{\stepcounter{fvnum}.fv\arabic{fvnum}}


\newenvironment{fvnote}{\stepcounter{fvnum}\color{blue} [\,\textit{Note:}\,}{\textit{\,---\,end note\fvid}\,]\color{black}}


\newcommand{\fverror}[1]{\stepcounter{fvnum}\color{red}{[\,\textit{Error:}\, #1 \textit{\,---\,end error\fvid}\,]}\color{black}}


% This command allows us to label margin notes with the corresponding paragraphsthey refer to and is based on \parabullnum from layout.tex
\makeatletter
\newcommand{\fvparabullnum}{%
				\arabic{chapter}.\arabic{section}% Dump out chaper.section
				\ifnum \value{subsection}>0 .\arabic{subsection}%
				\ifnum \value{subsubsection}>0 .\arabic{subsubsection}%
				\ifnum \value{paragraph}>0 .\arabic{paragraph}%
				\ifnum \value{subparagraph}>0 .\arabic{subparagraph}%
				\fi\fi\fi\fi%
				\ifnum \value{Paras}>0
				\ifnum \value{Bullets1}>0 \fi%
				\space p\arabic{Paras}%
...
%fvmargin Dumps comments in the margin
% - you must use \mbox within it if you need spaces within code and indentation
% - each line must end with % except for the last.
\newcommand{\fvmargin}[1]{%  <-- this % here is what prevents insertion of space in text!
	\stepcounter{fvnum}%
	\sidepar{%
		\smaller\color{blue}%
		[\textit{\fvparabullnum:}\space%
		#1 \textit{\,---\,end margin\mbox{\fvid}}]%
		\color{black}%
	}%
}
\newcommand{\fvmarginspace}[1] {\color{white} #1 \color{blue}}


%\newcommand{\addfv}[1]{\stepcounter{fvnum}\color{darkpurple}#1\fvid\color{black}}

%highlight and mark fv
%NOTE: You must wrap \ref{#1}, \cite in \mbox if it gives you an error of an extra }
%Note - to avoid having to wrap ref/cite and pageref in \mbox we use this hack as shown in http://tex.stackexchange.com/questions/139463/how-to-make-hl-highlighting-to-automatically-place-incompatible-commands-in
...
\newcommand{\fv}[1]{\stepcounter{fvnum}\hl{#1\fvid}}
\newcommand{\fvins}[1]{{\color{orange}#1\fvid}}
\newcommand{\fvhl}[1]{\stepcounter{fvnum}\hl{#1 \fvid}}

\newcommand{\fventerexample} {\color{darkpurple}[\,\textit{Example:}\space}
\newcommand{\fvexitexample}{\stepcounter{fvnum}\textit{\,---\,end example\fvid}\,]\space\color{black}}

\newenvironment{fvexample}{\color{darkpurple}[\,\textit{Example:}\space}{\stepcounter{fvnum}\textit{\,---\,end example\fvid}\,]\space\color{black}}

\newcommand{\fvpaper}[1]{\href{http://wg21.link/#1}{#1}}
\newcommand{\fvcwg}[1]{\href{http://wg21.link/cwg#1}{cwg#1}}
\newcommand{\fvgit}[1]{\href{https://github.com/cplusplus/draft/commit/#1}{git\StrLeft{#1}{5}...}}


\makeatletter                         % override '@' security
\let\fvfootnote\@fvundefined            % set fvfootnote to undefined
\global\newsavebox{\@fvtempfootboxa}   % must be global, to escape tables/figures
\newsavebox{\@fvtemplfootbox}
\newenvironment{fvfootnote}{%
	\unskip \footnotemark%    no space before the mark
	\color{darkpurple}%
	\stepcounter{fvnum}\unskip\textsuperscript{\fvid}%
	\normalfont%
	\footnotesize%           text size for rendering the footnote text
	\begin{lrbox}{\@fvtemplfootbox}%       temporarily save to local box
	}{%
	\end{lrbox}%
	\global\setbox\@fvtempfootboxa\hbox{\unhbox\@fvtemplfootbox}%  copy to global box
	\footnotetext{\unhbox\@fvtempfootboxa}%
}
\makeatother

\newcommand{\fvfootnotespace}[1] {\color{white} #1 \color{darkpurple}}

% save/restore current color: https://tex.stackexchange.com/questions/199688/how-to-change-the-colour-back-to-the-original-following-color
\makeatletter
\def\fvsavecolor{\edef\fvrestorecolor{\pdfliteral{\current@color}}}%pdftex version
\makeatother

%for placing spaces we change the color to white and insert the chracters.
\newcommand{\fvspace}[1] {\fvsavecolor \color{white} #1 \fvrestorecolor}
%%%%%%%%%%%%%%%% END FV Annotation Macros %%%%%%%%%%%%%%%%%%%%%%%%
