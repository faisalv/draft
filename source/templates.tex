...
\end{itemize}
\fvmargin{Just forming a specialization of a class, variable or alias (not even instantiating it, requires satisfaction of its constraints)}
\pnum
...
\begin{note}
\fvhl{Forming the name of a specialization of
a class template,
...
\begin{note}
\fvhl{Atomic constraints are formed by constraint normalization\iref{temp.constr.normal}.}
\tcode{E} is never a logical AND expression\iref{expr.log.and}
nor a logical OR expression\iref{expr.log.or}.
\end{note}

\fvmargin{
	-- Can think of concepts as macros for expressions - they let you get the exact same expression where you need it - and they are the only thing that let you do that.  every other spelling of an expression - though similar in spelling - is not the SAME expression.\newline\newline
	-- same expression - means same source level construct\newline\newline
	-- 'same expression' is not required when doing declaration matching for redeclaration purposes.
	\fvpaper{p0717}\newline\newline
    -----------------------------------------------------------------\newline\newline
		For a constraint to be satisfied, two things have to occur:\newline
		- substitution into the atomic-constraint (using the p-mapping and t-args) must succeed\newline
		- the atomic-constraints must evaluate to true\newline
		NOTE: if the atomic-constraint is not EXACTLY of type 'bool' an error occurs\newline
}
...
\fvmargin{\uline{NOTES ON PARAMETER MAPPING}:\newline
	\href{http://lists.isocpp.org/core/2017/10/3153.php}{cwg-reflector-discussion}\newline
	\newline -- think of parameter mappings as a way to substitute various non-canonical template-parameters to get the eventual requires expression into a canonical (0-0 clang rep) templater-parameter form (ignore eventual template arguments)
	\newline -- substitution during creation of 'parameter mappings' is DIFFERENT (ill-formed) from substitution within the constraint expression (SFINAE)\newline
	-- parameter mappings are non-empty and non-identity only if one concept references another and adds 'something' to the parameter - and creates levels of mapping of types until it gets to substitution with the atomic-constraints of a concept\newline\newline\newline
	\tcode{template<class T, class U> concept C1 = 	\newline
		sizeof(T**) == sizeof(U*);\newline
		template<class V, class W> concept C2 = C1<W*, V*>;\newline
	}\newline
	\term{NOTE: no \tcode{void f(X,Y)} since that would be beaten at the partial specialization stage before we even get to the constraint subsumption check.}\newline
	\tcode{template<class X, class Y> req C1<X*,Y*> void f(X*,Y*);  \#1\newline
		template<class X, class Y> req C2<X,Y> void f(X*,Y*);   \#2\newline
		template<class X, class Y> req C2<X,Y> void f(X*,Y*) requires true; \#3\newline}
	C1<X*,Y*>  should normalize to the same associated constraints as C2<int> - and the mapping enables that.  Consider:
	f((int*)0);
	\#1 : TArg : V = int*, Map: T -> V
	\#2 : TArg : V = int, Map: T -> V*  
	When satisfying, first substitute the parameter mapping into each atomic-constraint - and then the template argument for e.g. in \#2 : sizeof(V*) == sizeof(V**)
}
\pnum
...
\fvmargin{Subsumption is implication : \newline
To check if P --> Q:\newline 
  -- If P = (A AND B) and Q = (A OR B) then P implies Q because there exists an element of (A AND B) in (A OR B) it would also imply Q = A  or Q = B\newline
  -- A OR B does not imply A AND B\newline
  -- Transform P into an equivalent logical expression with (disjunctive)clauses \term{Pi} each separated by \tcode{'OR'}s - and each clauses' components are atomic constraints separated by \tcode{'AND'} (DNF): \tcode{(A AND B) OR (C AND D)}\newline
  -- Transform Q into an equivalent logical expression with (conjunctive)clauses \term {Qj} separated by \tcode{'AND'}s - and each clauses' components are atomic constraints separate by \tcode{'OR'} (CNF): \tcode{(A OR C) AND (A OR D) AND (B OR C) AND (B OR D)}.\newline
  -- Then ensure that each disjunctive clause \term{Pi} in P subsumes every conjunctive clause \term{Qi} in Q.\newline
  ----  \term{Pi} subsumes \term{Qi} if you find some atomic-constraint \term{Pia} (separated by \tcode{AND}) in \term{Qi}.\newline
  -- Disjunctive Normal Form: (A \& C) | (A \& D) | (B \& C) | (B \& D)\newline  
  -- Conjunctive Normal Form: (A | B) \& (C | D)\newline
This was implemented in gcc using a tactical theorem prover as described in ML for the working programmer.\newline\newline
- If P does not contain any '||'s, then use P\newline
- If P does not contain any parenthetical grouping, use P\newline
else convert it into an equivalent logical expression that is separated by '||'\newline
}
\pnum
...
\end{note}
\fvmargin{These rules can be summarized as follows, when applied to two dependent expressions in constraints:\newline\newline
...
template arguments, the expressions are “functionally equivalent but not equivalent”, and any program that results in such a comparison being attempted is ill-formed, with no diagnostic required.\newline\newline
