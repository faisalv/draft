...
\fvmargin{Daveed's Book:\newline
	...
	\newline (\textbf{Note: Template argument deduction occurs prior to Overload Resolution})
	\newline
	\begin{enumerate}
		\item The name is looked up to form an initial overload set.
		\item If necessary, \textbf{\underline{template argument deduction}}, substitution and constraint satisfaction occurs, which can cause some function template candidates to be discarded.
		\item Any candidate that doesn’t match the call at all (even after considering implicit
