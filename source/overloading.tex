...
\fvmargin{From Daveed's Book:\newline
	Calls through \textbf{function-pointers and	pointers-to-member-functions} do not need overload resolution.\newline\newline
	A call to a named function is broadly processed as follows:
	\newline 
	\newline
	\begin{enumerate}
		\item \textbf{Lookup} is used to form an overload set.
		\item \textbf{Template argument deduction \underline{(see steps at p\pageref{fv:template-argument-deduction-steps-cwg2369})}} can cause some templates to be discarded.  For function object calls - \textbf{function call operators and surrogate functions} (for non-template conversion functions)  are added.
		\item \textbf{Viability} of arguments (number, implicit conversions and default argument) is used to filter out candidates.
		\item \textbf{Overload resolution} is performed to find a best candidate. If there is one, it is selected; otherwise, the call is ambiguous.
		\begin{enumerate}
		...
		\end{enumerate}
		\item Accessibility (i.e. private etc) and Deletion (i.e. = delete) is checked.
	\end{enumerate}
}
